...
import { Activity, FileText, Pill, Utensils, Dumbbell, Bot, Calendar, Heart, Thermometer, Droplet } from 'lucide-react';
...
const patientData = {
  ...
  surgeryDescription: 'Complete replacement of damaged knee joint with prosthetic implant due to severe osteoarthritis.',
  riskLevel: 'High',
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card>
        ...
        <div className="grid grid-cols-3 gap-4">
          <VitalCard icon={Heart} label="Heart Rate" value="78 bpm" status="success" />
          <VitalCard icon={Droplet} label="Blood Pressure" value="145/92" status="warning" />
          <VitalCard icon={Thermometer} label="Temperature" value="37.2°C" status="success" />
        </div>
      </Card>
      ...
      <Card>
        ...
        <div className="space-y-3">
          <TaskRow icon={Pill} task="Administer pain medication" time="10:00 AM" status="success" />
          <TaskRow icon={Activity} task="Physical therapy session" time="2:00 PM" status="warning" />
          <TaskRow icon={Utensils} task="Low-sodium lunch" time="12:30 PM" status="success" />
          <TaskRow icon={FileText} task="Update recovery notes" time="4:00 PM" status="neutral" />
        </div>
      </Card>
    </div>
  </div>
  ...
  <Card>
    <div className="space-y-6">
      <div>
        ...
        <Badge variant="info" className="mt-2">{patientData.surgeryCategory}</Badge>
      </div>
...
const MedicalRecordsTab: React.FC = () => (
  <div className="space-y-6">
    <Card>
      ...
      <p className="text-[#475569]">Severe osteoarthritis of the right knee with significant cartilage degeneration. Patient reports chronic pain and limited mobility affecting daily activities.</p>
    </Card>
    
    <Card>
      <h3 className="mb-4">Doctor's Notes</h3>
      <div className="space-y-4">
        <NoteItem date="Nov 27, 2025" doctor="Dr. Sarah Johnson" note="Patient showing good progress. Pain management effective. Continue current protocol." />
        <NoteItem date="Nov 25, 2025" doctor="Dr. Michael Chen" note="Post-operative check completed. Incision healing well. No signs of infection." />
        <NoteItem date="Nov 22, 2025" doctor="Dr. Sarah Johnson" note="Physical therapy initiated. Patient tolerating exercises well." />
      </div>
    </Card>
      ...
      <table className="w-full">
        <thead className="bg-[#F8FAFC]">
          <tr className="border-b border-[#E2E8F0]">
            <th className="text-left px-6 py-4 text-[#475569]">Admission Date</th>
            <th className="text-left px-6 py-4 text-[#475569]">Expected Discharge</th>
            <th className="text-left px-6 py-4 text-[#475569]">Ward</th>
            <th className="text-left px-6 py-4 text-[#475569]">Attending Doctor</th>
  ...
  <div className="space-y-6">
    <Card>
      ...
      <div className="space-y-3">
        ...
        <InfoRow label="Notes" value="Focus on lean proteins and vegetables. Limit sodium intake to reduce swelling." />
      </div>
    </Card>
    ...
    <Card>
      ...
      <div className="space-y-4">
        <MealRow meal="Breakfast" food="Oatmeal with berries, scrambled eggs, herbal tea" />
        <MealRow meal="Lunch" food="Grilled chicken salad with olive oil dressing, whole grain bread" />
        <MealRow meal="Dinner" food="Baked salmon with steamed vegetables, quinoa" />
...
const ActivitiesTab: React.FC = () => (
  <div className="space-y-6">
    ...
    <Card>
      ...
      <p className="text-[#475569] mb-4">Ask AI if an activity is safe for this patient's current condition.</p>
...
const AIInsightsTab: React.FC = () => (
  <div className="space-y-6">
    <Card>
      ...
      <div className="space-y-4">
        <div className="p-4 rounded-lg bg-[#FEE2E2] border border-[#FECACA]">
          ...
          <p className="text-[#991B1B] text-[14px]">Patient's blood pressure has been consistently elevated. Recommend consultation with cardiologist.</p>
        </div>
        ...
        <div className="p-4 rounded-lg bg-[#FEF9C3] border border-[#FDE047]">
          ...
          <p className="text-[#854D0E] text-[14px]">Recovery is slightly slower than typical timeline. Continue monitoring closely.</p>
        </div>
      </div>
    </Card>
    ...
    <Card>
      ...
      <p className="text-[#475569] mb-4">Based on current progress, AI predicts:</p>
      <ul className="list-disc list-inside space-y-2 text-[#475569]">
    ...
    <Card>
      ...
      <ul className="space-y-3">
        <li className="flex items-start gap-3">
          <div className="w-6 h-6 rounded-full bg-[#2563EB] text-white flex items-center justify-center text-[12px] flex-shrink-0 mt-1">1</div>
          <div>
            <div className="text-[#0F172A] mb-1">Schedule cardiology consultation</div>
            <div className="text-[14px] text-[#475569]">Due to persistent elevated blood pressure readings</div>
          </div>
        </li>
        <li className="flex items-start gap-3">
          <div className="w-6 h-6 rounded-full bg-[#2563EB] text-white flex items-center justify-center text-[12px] flex-shrink-0 mt-1">2</div>
          <div>
            <div className="text-[#0F172A] mb-1">Increase physical therapy frequency</div>
            <div className="text-[14px] text-[#475569]">To accelerate recovery progress</div>
          </div>
        </li>
        <li className="flex items-start gap-3">
          <div className="w-6 h-6 rounded-full bg-[#2563EB] text-white flex items-center justify-center text-[12px] flex-shrink-0 mt-1">3</div>
          <div>
            <div className="text-[#0F172A] mb-1">Review pain medication dosage</div>
            <div className="text-[14px] text-[#475569]">Patient may benefit from adjusted protocol</div>
          </div>
        </li>
      </ul>
    </Card>
...
const VitalCard: React.FC<{ icon: any; label: string; value: string; status: 'success' | 'warning' | 'error' }> = ({ icon: Icon, label, value, status }) => {
  const colors = {
    success: '#22C55E',
    warning: '#FACC15',
    error: '#EF4444',
...
const TaskRow: React.FC<{ icon: any; task: string; time: string; status: 'success' | 'warning' | 'neutral' }> = ({ icon: Icon, task, time, status }) => {
  ...
  return (
    <div className="flex items-center gap-3 p-2 rounded hover:bg-[#F8FAFC]">
      ...
      <Badge variant={getBadgeVariant()} size="sm">
        {status === 'success' ? 'Done' : status === 'warning' ? 'Pending' : 'Scheduled'}
      </Badge>
    </div>
  );
};

const NoteItem: React.FC<{ date: string; doctor: string; note: string }> = ({ date, doctor, note }) => (
  <div className="p-4 rounded-lg border border-[#E2E8F0]">
...
const FileCard: React.FC<{ name: string; date: string }> = ({ name, date }) => (
  <div className="p-4 rounded-lg border border-[#E2E8F0] hover:border-[#2563EB] transition-colors cursor-pointer" style={{ height: '120px' }}>
...
const ActivityCard: React.FC<{ type: 'allowed' | 'restricted'; activity: string }> = ({ type, activity }) => (
  <div 
    className={`p-4 rounded-lg border ${type === 'allowed' ? 'border-[#BBF7D0] bg-[#F0FDF4]' : 'border-[#FECACA] bg-[#FEF2F2]'}`}
    style={{ height: '80px' }}
  >
    <div className={`text-[14px] ${type === 'allowed' ? 'text-[#166534]' : 'text-[#991B1B]'}`}>
      {activity}
    </div>
  </div>
);

const AppointmentRow: React.FC<{ date: string; time: string; doctor: string; type: string; status: 'confirmed' | 'scheduled' | 'completed' }> = ({ date, time, doctor, type, status }) => {
  const getBadge = () => {
  ...
  return (
    <div className="flex items-center gap-4 p-4 rounded-lg border border-[#E2E8F0]">
      ...
      <div className="flex-1">
        ...
        <div className="text-[14px] text-[#475569]">{date} at {time} • {doctor}</div>
      </div>
      {getBadge()}
    </div>
  );
