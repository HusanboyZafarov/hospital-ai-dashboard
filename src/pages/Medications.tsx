...
const medications = [
  { name: 'Oxycodone', dosage: '5mg', frequency: 'Every 6 hours', startDate: 'Nov 16, 2025', endDate: 'Nov 30, 2025' },
  { name: 'Cephalexin', dosage: '500mg', frequency: 'Twice daily', startDate: 'Nov 16, 2025', endDate: 'Nov 26, 2025' },
  { name: 'Enoxaparin', dosage: '40mg', frequency: 'Once daily', startDate: 'Nov 16, 2025', endDate: 'Dec 1, 2025' },
  { name: 'Ibuprofen', dosage: '400mg', frequency: 'As needed', startDate: 'Nov 16, 2025', endDate: 'Dec 10, 2025' },
];
          ...
          <table className="w-full">
            ...
            <tbody>
              {medications.map((med, index) => (
                <tr key={index} className="border-b border-[#E2E8F0] hover:bg-[#F8FAFC]" style={{ height: '64px' }}>
      ...
      <Card>
        ...
        <div className="mb-2 ml-[140px] flex">
          {Array.from({ length: 24 }, (_, i) => (
            <div key={i} className="flex-1 text-center text-[13px] text-[#475569] min-w-[40px]">
              {i}:00
            </div>
          ))}
        </div>
          ...
          {timelineData.map((item, index) => (
            <div key={index} className="flex items-center">
              ...
              <div className="flex-1 flex relative h-12 bg-[#F8FAFC] rounded">
                {Array.from({ length: 24 }, (_, i) => (
                  <div key={i} className="flex-1 border-r border-[#E2E8F0] min-w-[40px]"></div>
                ))}
                {item.schedule.map((slot, slotIndex) => {
                  const hour = parseInt(slot.time.split(':')[0]);
                  const color = slot.status === 'taken' ? '#2563EB' : slot.status === 'missed' ? '#EF4444' : '#E2E8F0';
